...
function buildHeatmapYearSection(year, dayCountMap) {
    ...
    // Compute each month's start-week; span = nextMonth.startWeek − thisMonth.startWeek
    const monthStartWeekArr = [];
    const monthStartWeeks   = new Set(); // week columns that start a new month (skip Jan)
    for (let m = 0; m < 12; m++) {
        const startWeek = Math.floor(Math.round((new Date(year, m, 1) - gridStart) / 86400000) / 7);
        monthStartWeekArr.push(startWeek);
        if (m > 0) monthStartWeeks.add(startWeek); // Feb–Dec only
    }
    const totalWeeks = totalDays / 7;
    const monthSpans = MONTH_SHORT.map((name, m) => ({
        name,
        weekSpan: (m < 11 ? monthStartWeekArr[m + 1] : totalWeeks) - monthStartWeekArr[m],
    }));
