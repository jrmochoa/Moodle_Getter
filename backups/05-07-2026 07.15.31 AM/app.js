...
function updateStickyCourseBanner() {
    ...
    if (!tabBarRect || tabBarRect.top > 1) { stickyCourseBanner.style.display = 'none'; return; }
    const threshold = tabBarRect.bottom;
    const seps = Array.from(document.querySelectorAll('tr[data-sep-idx]'));
    let active = null;
    for (const sep of seps) {
        if (sep.getBoundingClientRect().top < threshold) active = sep;
    }
    if (active) {
