...
function updateStickyCourseBanner() {
    ...
    const sepTop = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--sep-top')) || (tabBar ? tabBar.offsetHeight : 48);
    const seps = Array.from(document.querySelectorAll('tr[data-sep-idx]'));
    let active = null;
    for (const sep of seps) {
        if (sep.getBoundingClientRect().bottom < sepTop) active = sep;
    }
    if (active) {
