...
- **Trigger** — `#heatmapBtn` in the toolbar (calendar icon); disabled until a fetch succeeds; enabled via `setDownloadDisabled(heatmapBtn, false)` after fetch
- **Data source** — `currentRows` at the moment the button is clicked; respects active tab + type + date filters; groups by `courseGroupIndex`
- **State variables** — `heatmapCourses[]` (`{courseName, rows}`), `heatmapCourseIdx` (current index)
- **Functions** — `openHeatmapModal()`, `closeHeatmapModal()`, `renderHeatmapForCourse(idx)`, `buildHeatmapYearSection(year, dayCountMap)`, `updateHeatmapNav()`
