...
function renderTable(rows) {
    ...
    rows.forEach(row => {
        ...
        tr.append(tdNum, tdDate, tdUser, tdName, tdType, tdSubject, tdStrategy, tdLog);
        frag.appendChild(tr);
    });
...
function updateTabUI() {
    ...
    if (activeTab === 'teachers') {
        tTeachers.style.color        = '#a78bfa';
        tTeachers.style.borderBottom = '2px solid #a78bfa';
        cTeachers.style.background   = 'rgba(139,92,246,.14)';
        cTeachers.style.color        = '#a78bfa';

        tOer.style.color        = 'rgba(148,163,184,.45)';
        tOer.style.borderBottom = '2px solid transparent';
        cOer.style.background   = 'rgba(148,163,184,.08)';
        cOer.style.color        = 'rgba(148,163,184,.45)';
    } else {
        tOer.style.color        = '#a78bfa';
        tOer.style.borderBottom = '2px solid #a78bfa';
        cOer.style.background   = 'rgba(139,92,246,.14)';
        cOer.style.color        = '#a78bfa';

        tTeachers.style.color        = 'rgba(148,163,184,.45)';
        tTeachers.style.borderBottom = '2px solid transparent';
        cTeachers.style.background   = 'rgba(148,163,184,.08)';
        cTeachers.style.color        = 'rgba(148,163,184,.45)';
    }
...
function exportCSV(rows, filename, label = '') {
    ...
    const hdr  = ['Date', 'User', 'Activity Name', 'Activity Type', 'Subject', 'Strategy'];
...
downloadBtn.addEventListener('click', () => {
    ...
    const isOer   = activeTab === 'oer';
    const label   = isOer ? 'OER' : 'Teachers Creation';
    const suffix  = isOer ? 'oer' : 'teachers_creation';
    const ids     = [...new Set(rows.map(r => r.courseId))].join('-');
    exportCSV(rows, `moodle_${ids}_${suffix}.csv`, label);
});
