...
function renderTable(rows) {
    ...
    if (!showSeparators) {
        ...
        // Ordered unique courses from full tableData (includes courses filtered to 0 rows)
        const courseGroups = [];
        const seenGroups = new Set();
        tableData.forEach(r => {
            if (!seenGroups.has(r.courseGroupIndex)) {
                seenGroups.add(r.courseGroupIndex);
                courseGroups.push(r);
            }
        });
        ...
        courseGroups.forEach(cg => {
            ...
            if (!courseRows.length) {
                ...
                emptyTd.textContent = 'No activities for this course.';
...
function openHeatmapModal(targetGroupIndex = null) {
    // Build course list from all tableData so courses with 0 filtered rows still appear
    const courseGroupsOrdered = [];
    const seenGroups = new Set();
    tableData.forEach(r => {
        if (!seenGroups.has(r.courseGroupIndex)) {
            seenGroups.add(r.courseGroupIndex);
            courseGroupsOrdered.push({ courseGroupIndex: r.courseGroupIndex, courseName: r.courseName });
        }
    });
    if (!courseGroupsOrdered.length) return;
    ...
    heatmapCourses = courseGroupsOrdered.map(g => ({
    ...
    if (targetGroupIndex !== null) {
        const found = courseGroupsOrdered.findIndex(g => g.courseGroupIndex === targetGroupIndex);
