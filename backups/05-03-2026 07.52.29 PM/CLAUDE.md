...
**Course separator rows** — shown in unsorted view only (`sortCol === null`); always rendered for every course in `tableData`, even if the course has zero rows after filtering — in that case a "No activities for this course." empty-state row follows the separator. Each separator carries a `.course-sep-heatmap-btn` (`book-open-check.svg`) that opens the heatmap modal pre-selected to that course.
