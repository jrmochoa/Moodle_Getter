...
## Course Heatmap Popup

- **Trigger** — `mouseenter` on a `course-sep-cell` row; 150 ms hide delay on `mouseleave` so the cursor can move into the popup without it closing
- **Data source** — `currentRows` filtered to the hovered `courseGroupIndex`; reflects active tab + type + date filters at the moment of hover; auto-closes on any `renderTable()` call (i.e. any filter/sort change)
- **Year sections** — one section per calendar year found in visible rows, sorted ascending; each section has a 12-column month grid (Jan → Dec)
- **Cell states**: `heatmap-cell--has-data` (green, count ≥ 1) · `heatmap-cell--no-data` (red, count = 0, past/current month) · `heatmap-cell--future` (grey, month is beyond today)
- **State variables** — `heatmapPopup` (DOM ref), `heatmapHideTimer` (timeout ID)
- **Functions** — `closeHeatmapPopup()`, `showHeatmapPopup(sepTr, courseGroupIndex)`
- **Constant** — `HEATMAP_MONTHS` (3-letter abbreviations, used for grid labels and `title` tooltips)
- Popup is `position:fixed`, `z-index:9999`, width 314 px; positioned below the row with fallback to above if viewport space is insufficient
