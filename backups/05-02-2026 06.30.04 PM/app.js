...
function renderTable(rows) {
    ...
    rows.forEach(row => {
        ...
        if (showSeparators && row.courseGroupIndex !== lastGroupIndex) {
            ...
            const _cgIdx = row.courseGroupIndex;
            sepTr.addEventListener('mouseenter', () => {
                clearTimeout(heatmapHideTimer);
                showHeatmapPopup(sepTr, _cgIdx);
            });
            sepTr.addEventListener('mouseleave', () => {
                heatmapHideTimer = setTimeout(closeHeatmapPopup, 150);
            });
        }
        ...
        tdDate.innerHTML = `<span class="date-chip${dark ? '' : ' date-chip--light'}">${escapeHtml(row.date)}</span>${badge}`;
        ...
        tdType.innerHTML = `<span class="activity-type-chip${dark ? '' : ' activity-type-chip--light'}">${escapeHtml(row.activityType)}</span>`;
...
function showHeatmapPopup(sepTr, courseGroupIndex) {
    closeHeatmapPopup();
    const courseRows = currentRows.filter(r => r.courseGroupIndex === courseGroupIndex);
    ...
    const years = [...yearSet].map(Number).sort((a, b) => a - b);
    ...
    years.forEach(year => {
        ...
        section.className = 'heatmap-year-section';
        ...
        for (let m = 1; m <= 12; m++) {
            ...
            const future = year > todayYear || (year === todayYear && m > todayMonth);
            const cell   = document.createElement('div');
            const state  = future ? 'future' : count > 0 ? 'has-data' : 'no-data';
            cell.className = `heatmap-cell heatmap-cell--${state}`;
            cell.textContent = future ? '' : String(count);
            cell.title = future
                ? `${HEATMAP_MONTHS[m - 1]} ${year}: future`
                : `${HEATMAP_MONTHS[m - 1]} ${year}: ${count} entr${count === 1 ? 'y' : 'ies'}`;
        ...
        HEATMAP_MONTHS.forEach(mn => {
            const lbl = document.createElement('div');
            lbl.className = 'heatmap-month-label';
            lbl.textContent = mn[0];
    ...
    popup.addEventListener('mouseenter', () => clearTimeout(heatmapHideTimer));
    ...
    const rect = sepTr.getBoundingClientRect();
    const gap  = 6;
    popup.style.left = rect.left + 'px';
    const ph = popup.offsetHeight;
    if (window.innerHeight - rect.bottom - gap >= ph) {
        popup.style.top = (rect.bottom + gap) + 'px';
    } else {
        popup.style.top = (rect.top - ph - gap) + 'px';
    }
    const pw       = popup.offsetWidth;
    const leftEdge = parseFloat(popup.style.left);
    if (leftEdge + pw > window.innerWidth - 8) {
