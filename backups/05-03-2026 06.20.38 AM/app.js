...
function renderTable(rows) {
    ...
    rows.forEach(row => {
        ...
        if (showSeparators && row.courseGroupIndex !== lastGroupIndex) {
            ...
            sepTd.innerHTML = `<a href="${escapeHtml(row.courseBaseUrl)}/course/view.php?id=${row.courseId}" target="_blank" rel="noopener" class="course-sep-link">
...
function openHeatmapModal() {
    ...
    // Group rows by courseGroupIndex, preserving encounter order
    const courseMap = new Map();
    currentRows.forEach(r => {
        if (!courseMap.has(r.courseGroupIndex)) {
            courseMap.set(r.courseGroupIndex, { courseName: r.courseName, rows: [] });
        }
        courseMap.get(r.courseGroupIndex).rows.push(r);
    });
    heatmapCourses = [...courseMap.values()];
...
function renderHeatmapForCourse(idx) {
    ...
    const years = [...new Set(Object.keys(dayCountMap).map(k => parseInt(k)))].sort((a, b) => b - a);

    heatmapContent.innerHTML = '';
    years.forEach(year => heatmapContent.appendChild(buildHeatmapYearSection(year, dayCountMap)));
    updateHeatmapNav();
}

function buildHeatmapYearSection(year, dayCountMap) {
    const CELL = 13;
    const GAP  = 2;
    const STEP = CELL + GAP; // 15px per week column
    ...
    // Monday-based offset of Jan 1 (Mon=0 … Sun=6)
    const jan1Mon  = (jan1.getDay()  + 6) % 7;
    const dec31Mon = (dec31.getDay() + 6) % 7;

    const gridStart = new Date(year, 0,  1 - jan1Mon);          // Monday ≤ Jan 1
    const gridEnd   = new Date(year, 11, 31 + (6 - dec31Mon));  // Sunday ≥ Dec 31

    const totalDays = Math.round((gridEnd - gridStart) / 86400000) + 1;

    // Compute each month's week-span for the label row + first-week column for separators
    const monthSpans      = [];
    const monthStartWeeks = new Set(); // week columns that start a new month (skip Jan)
    for (let m = 0; m < 12; m++) {
        const first     = new Date(year, m, 1);
        const last      = new Date(year, m + 1, 0);
        const startWeek = Math.floor(Math.round((first - gridStart) / 86400000) / 7);
        const endWeek   = Math.floor(Math.round((last  - gridStart) / 86400000) / 7);
        monthSpans.push({ name: MONTH_SHORT[m], weekSpan: endWeek - startWeek + 1 });
        if (m > 0) monthStartWeeks.add(startWeek); // Feb–Dec only
    }
    ...
    // Day-of-week label column (Mon / Wed / Fri / Sun)
    const dayCol = document.createElement('div');
    dayCol.className = 'hm-day-col';
    ...
    // Right side: grid + month labels
    const rightDiv = document.createElement('div');
    rightDiv.className = 'hm-right';
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        if (!inYear) {
            ...
            cell.title = `${d.getDate()} ${MONTH_NAMES[d.getMonth()]} ${d.getFullYear()}`;
        } else {
            const key   = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
            const count = dayCountMap[key] || 0;
            const level = count === 0 ? 0 : count <= 3 ? 1 : count <= 6 ? 2 : count <= 10 ? 3 : 4;
            cell.classList.add(`hm-level-${level}`);
            const ds = `${d.getDate()} ${MONTH_NAMES[d.getMonth()]} ${d.getFullYear()}`;
            cell.title = `${count} module${count === 1 ? '' : 's'} on ${ds}`;
        }

        // Subtle left-edge separator at the first week column of each month (Feb–Dec)
        if (inYear && monthStartWeeks.has(Math.floor(i / 7))) {
            cell.classList.add('hm-cell--month-start');
        }
    ...
    // Month label row (widths computed from week spans)
    const monthRow = document.createElement('div');
    monthRow.className = 'hm-month-row';
    monthSpans.forEach(({ name, weekSpan }) => {
        const lbl = document.createElement('div');
        lbl.className = 'hm-month-label';
        lbl.textContent = name;
        lbl.style.width = (weekSpan * STEP) + 'px';
...
function updateHeatmapNav() {
    ...
    if (hasPrev) heatmapPrevBtn.innerHTML = `<img src="src/icons/chevron-left.svg" width="13" height="13" class="icon-img" alt=""> ${escapeHtml(heatmapCourses[idx - 1].courseName)}`;
    if (hasNext) heatmapNextBtn.innerHTML = `${escapeHtml(heatmapCourses[idx + 1].courseName)} <img src="src/icons/chevron-right.svg" width="13" height="13" class="icon-img" alt="">`;
}
...
fetchBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        setBtnSuccess('✓ Done');
        setDownloadDisabled(heatmapBtn, allData.length === 0);
