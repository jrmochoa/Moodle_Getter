...
**Snapshot variables** (`snapshotAllData`, `snapshotTeacherData`, `snapshotOerData`, `snapshotHiddenTypes`, `snapshotHiddenYears`, `snapshotHiddenMonths`) are saved immediately after the tab split and default filter are set, and are used exclusively by `resetTable()` to restore the default view without refetching.
...
Two independent column filters operate on the rendered rows. Both follow the same pattern: a `Set` of hidden keys, a dropdown built dynamically from the active tab's data, an outside-click listener that removes the dropdown, and a `filter-active` CSS class on the icon when the set is non-empty.

| Filter | State vars | Icon element | Toggle function |
|---|---|---|---|
| Activity Type | `hiddenTypes` | `#typeFilterIcon` | `toggleFilterDropdown` |
| Date (Year + Month) | `hiddenYears`, `hiddenMonths` | `#dateFilterIcon` | `toggleDateFilterDropdown` |

**Filter application order** inside `applyAndRender()`: sort → type filter → date filter.

**Date filter details:**
- Date column format is `MM/DD/YYYY`. Year = `parts[2]`, month = `parts[0]` (zero-padded string, e.g. `"01"`).
- `hiddenYears` stores year strings (e.g. `"2024"`). `hiddenMonths` stores zero-padded month strings (e.g. `"01"`).
- A row is visible only when its year is NOT in `hiddenYears` AND its month is NOT in `hiddenMonths` (AND logic).
- The dropdown scans `getActiveRows()` (full tab data, ignoring any existing date filter) so hidden dates remain accessible.
- Years display as-is; months display as full names via the `MONTH_NAMES` constant.
- Both filters are cleared (empty Sets) on every new fetch and by `resetTable()`.
- CSV export in `downloadBtn` applies both filters identically to `applyAndRender()`.

**Reset Table** restores `hiddenYears` and `hiddenMonths` from `snapshotHiddenYears` / `snapshotHiddenMonths` (both are empty Sets after a fresh fetch).

### 7. Date Confidence Hierarchy
